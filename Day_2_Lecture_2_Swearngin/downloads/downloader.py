...
DATASET_VINS_URL = (
    "https://drive.google.com/file/d/1ucti2ujNJgKV-Ts_mXALA7BZ97iPv8h6/view"
)
...
def download_rico(tmp_path="tmp", dataset_path="rico"):
    ...
    os.rename(
        os.path.join(extract_path, "combined"), os.path.join(dataset_path, "combined")
    )
...
def download_vins(tmp_path="tmp", dataset_path="vins"):
    ...
    gdown.download(
        DATASET_VINS_URL,
        output=os.path.join(tmp_path, "VINS Dataset.zip"),
        fuzzy=True,
        use_cookies=False,
    )
    ...
    cmd = [
        "7z",
        "x",
        os.path.join(tmp_path, "VINS Dataset.zip"),
        "-o" + str(extract_path),
    ]
    sp = subprocess.Popen(cmd)
    sp.communicate()
...
def download_boxes_gdown(tmp_path="tmp", dataset_path="webui-boxes"):
    ...
    gdown.download(
        DATASET_BOXES_URL,
        output=os.path.join(tmp_path, "all_boxes.zip"),
        fuzzy=True,
        use_cookies=False,
    )
...
def download_enrico(
    tmp_path="tmp",
    dataset_path="enrico",
    screenclassification_metadata_path="../metadata/screenclassification",
):
    if not os.path.exists(tmp_path):
        os.makedirs(tmp_path)
    ...
    os.rename(
        os.path.join(extract_path, "screenshots"),
        os.path.join(dataset_path, "screenshots"),
    )
    shutil.rmtree(tmp_path)
    ...
    metadata_output_path = os.path.join(
        screenclassification_metadata_path, "design_topics.csv"
    )
...
def download_metadata_gdown(metadata_key, metadata_path="../metadata"):
    ...
    gdown.download_folder(
        METADATA_GDRIVE_URLS[metadata_key],
        output=os.path.join(metadata_path, metadata_key),
        use_cookies=False,
    )
...
def download_dataset_gdown(dataset_key, tmp_path="tmp", dataset_path="ds"):
    ...
    if not os.path.exists(os.path.join(tmp_path, dataset_key)):
        gdown.download_folder(
            DATASET_GDRIVE_URLS[dataset_key],
            output=os.path.join(tmp_path, dataset_key),
            use_cookies=False,
        )
...
def download_model_gdown(model_name, model_key, model_path="checkpoints"):
    ...
    gdown.download(
        MODEL_GDRIVE_URLS[model_name][model_key],
        output=os.path.join(model_path, model_key),
        fuzzy=True,
        use_cookies=False,
    )
